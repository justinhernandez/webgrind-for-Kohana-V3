...
<head>
	...
	<?= html::script(WEBGRIND.'media/js/webgrind.js') ?>
	<?= html::stylesheet(WEBGRIND.'media/styles/style') ?>
	<script type="text/javascript" charset="utf-8">
		var fileUrlFormat = '<?= Kohana::config('webgrind.fileUrlFormat'); ?>';
		...
		function update(specificFile){
			...
			$.getJSON("<?= webgrind::url() ?>",
				...
				function(data){
                    ...
                    $("#breakdown").html(
                        '<img src="<?= webgrind::img("gradient_left.png") ?>" height="20" width="10">'+
                        '<img src="<?= webgrind::img("gradient_blue.png") ?>" height="20" width="'+Math.floor(data.breakdown['internal']/breakdown_sum*300)+'">'+
                        '<img src="<?= webgrind::img("gradient_grey.png") ?>" height="20" width="'+Math.floor(data.breakdown['include']/breakdown_sum*300)+'">'+
                        '<img src="<?= webgrind::img("gradient_green.png") ?>" height="20" width="'+Math.floor(data.breakdown['class']/breakdown_sum*300)+'">'+
                        '<img src="<?= webgrind::img("gradient_orange.png") ?>" height="20" width="'+Math.floor(data.breakdown['user']/breakdown_sum*300)+'">'+
                        '<img src="<?= webgrind::img("gradient_right.png") ?>" height="20" width="10">'+
                        '<div title="internal functions, include/require, class methods and procedural functions." style="background:url(<?= webgrind::img("gradient_markers.png") ?>);position:relative;top:-20px;left:10px;width:301px;height:19px"></div>'
                    );
		...
		function callTableRow(nr,data){
			return '<tr> \
						<td>'
						+($("#callinfo_area_"+data.functionNr).length ? '<img src="<?= webgrind::img("right.gif") ?>">&nbsp;&nbsp;<a href="javascript:openCallInfo('+data.functionNr+')">'+data.callerFunctionName+'</a>' : '<img src="<?= webgrind::img("blank.gif") ?>">&nbsp;&nbsp;'+data.callerFunctionName)
						+ ' @ '+data.line+'</td> \
						<td class="nr">'+data.callCount+'</td> \
						<td class="nr">'+data.summedCallCost+'</td> \
						<td><a title="Open file and show line" href="'+sprintf(fileUrlFormat,data.file,data.line)+'" target="_blank"><img src="<?= webgrind::img("file_line.png") ?>" alt="O"></a></td> \
					</tr>';
		...
		function functionTableRow(data){
			openLink = (data.file=='php:internal')?'':'<a title="Open file" href="'+sprintf(fileUrlFormat,data.file,-1)+'" target="_blank"><img src="<?= webgrind::img("file.png") ?>" alt="O"></a>';
			return '<tr> \
			            <td> \
			                <img src="<?= WEBGRIND.'media/' ?>img/call_'+data.kind+'.png" title="'+data.humanKind+'"> \
			            </td> \
						<td> \
							<a href="javascript:toggleCallInfo('+data.nr+')"> \
								<img id="fold_marker_'+data.nr+'" src="<?= webgrind::img("right.gif") ?>">&nbsp;&nbsp;'+data.functionName+' \
							</a> \
		...
		function checkVersion(){
			...
			$.getJSON("<?= webgrind::url() ?>",
				{'op':'version_info'},
				function(data){
					if(data.latest_version><?php echo webgrind::$webgrindVersion ?>){
		...
		$(document).ready(function() { 
			...
			<?php if(Kohana::config('webgrind.checkVersion')):?>
			setTimeout(checkVersion,100);
			<?php endif?>
			
			// load latest file onload
			<?php if (@$_GET['autoload_latest']==1): ?>
			update();
			<?php endif; ?>
			
			// reload data on form changes
			$("#options form").change(function(){
				update();
			});
...
<body>
    <div id="head">
        <div id="logo">
            <h1>webgrind<sup style="font-size:10px">v<?php echo webgrind::$webgrindVersion ?></sup></h1>
            ...
            <form method="get" onsubmit="update();return false;">
            	...
            	<div style="float:right;">
            		...
            		<select id="costFormat" name="costFormat">
            			<?php $cost_format = Kohana::config('webgrind.defaultCostformat'); // just to reduce calls ?>
            		    <option value="percent" <?php echo ($cost_format=='percent') ? 'selected' : ''?>>percent</option>
            		    <option value="msec" <?php echo ($cost_format=='msec') ? 'selected' : ''?>>milliseconds</option>
            		    <option value="usec" <?php echo ($cost_format=='usec') ? 'selected' : ''?>>microseconds</option>
            		</select>
            	</div>
            	<div style="float:right;">
        			...
        			<img class="list_reload" src="<?= webgrind::img("reload.png") ?>" onclick="reloadFilelist()">
            	</div>
            	<div style="float:right">
            		<label style="margin:0 5px">Show</label>
            		<select id="showFraction" name="showFraction">
            			<?php for($i=100; $i>0; $i-=10):?>
            			<option value="<?php echo $i/100?>" <?php if ($i==Kohana::config('webgrind.defaultFunctionPercentage')):?>selected="selected"<?php endif;?>><?php echo $i?>%</option>
            			<?php endfor;?>
            		</select>
            	</div>
            	<div style="clear:both;"></div>    	
            	<div style="margin:0 70px">
            	    <input type="checkbox" name="hideInternals" value="1" <?php echo (Kohana::config('webgrind.defaultHideInternalFunctions')==1) ? 'checked' : ''?> id="hideInternals">
