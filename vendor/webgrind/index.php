...
// for working with Kohana v3
require Kohana::find_file('vendor/webgrind', 'config');
require Kohana::find_file('vendor/webgrind/library', 'FileHandler');
    ...
    switch(get('op')){
        ...
        case 'function_graph':
		    ...
		    if (!file_exists($filename)) {
				shell_exec(Webgrind_Config::$pythonExecutable.' '.Webgrind_Config::$gprof2dotPath.' -n '.$showFraction.' -f callgrind '.Webgrind_Config::xdebugOutputDir().''.$dataFile.' | '.Webgrind_Config::$dotExecutable.' -Tpng -o ' . $filename);			
			}
			readfile($filename);
		break;
    	case 'version_info':
